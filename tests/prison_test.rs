...
fn prison_returns_the_correct_index_for_inserted_values() {
    ...
    let idx_1 = prison.insert(val_1);
    let idx_2 = prison.insert(val_2);
    let idx_3 = prison.insert(val_3);
...
fn pushing_to_prison_at_max_capacity_while_not_visiting_is_fine() {
    ...
    assert!(prison.visit(0, |val| {
        assert_eq!(*val, Var::F64(3.1415), "Value inserted into prison did not match value during visit");
        assert!(prison.insert(Var::Null).is_ok());
        assert!(prison.insert(Var::Null).is_ok());
        assert!(prison.insert(Var::Null).is_ok());
        assert!(prison.insert(Var::Null).is_ok());
    }).is_ok(), "Prison could not access first cell");
    assert!(prison.insert(Var::F64(9.9999)).is_ok());
...
fn pushing_to_prison_at_max_capacity_while_visiting_errors() {
    ...
    assert!(prison.visit(0, |val| {
        ...
        assert!(prison.insert(Var::Null).is_ok());
        assert!(prison.insert(Var::Null).is_ok());
        assert!(prison.insert(Var::Null).is_err(), "Prison allowed push while at max capaity AND visiting");
    }).is_ok(), "Prison could not access first cell");
}
...
fn prison_pop_method_errors_only_if_last_value_being_visited() {
    let mut prison = Prison::new();
    prison.insert(Var::F64(1.0));
    prison.insert(Var::F64(2.0));
    prison.insert(Var::F64(3.0));
    assert!(prison.visit(2, |val| {
        ...
        assert!(prison.remove().is_err(), "Prison allowed pop() while last index was visited");
    }).is_ok(), "Prison could not access last cell");
    assert!(prison.visit(1, |val| {
        assert!(prison.remove().is_ok(), "Prison did not allow pop() while second-to-last index was visited");
...
fn prison_visit_many_allows_access_to_disjoint_cells() {
    let mut prison = Prison::new();
    prison.insert(Var::F64(0.0));
    prison.insert(Var::F64(1.0));
    prison.insert(Var::F64(2.0));
    prison.insert(Var::F64(3.0));
    prison.insert(Var::F64(4.0));
    prison.insert(Var::F64(5.0));
